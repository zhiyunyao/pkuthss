...
本模板使用\verb|biblatex-gb7714-2015|宏包\footnote{\url{https://github.com/hushidong/biblatex-gb7714-2015}}进行参考文献管理与格式化，并提供相应的个性化配置方法。
...
maxbibnames=6,gbnamefmt=lowercase]{biblatex}
\end{Verbatim}

\subsection{正文中的标注格式个性化配置}

首先可以自定义“著者-出版年制”标注方式下，正文中的标注显示的作者数。
\begin{itemize}
...
\begin{table*}[htb]
    ...
    \begin{tabular}{ccccc}
        \toprule[1.5pt]
        \textbf{maxcitenames} & \textbf{赵} & \textbf{赵, 钱} & \textbf{赵, 钱, 孙} & \textbf{赵, 钱, 孙, 李} \\
        \midrule
        1 & 赵 & 赵等 & 赵等 & 赵等 \\
        \midrule
        2 & 赵 & 赵和钱 & 赵等 & 赵等 \\
        ...
        3 & 赵 & 赵和钱 & 赵, 钱和孙 & 赵等 \\
        \midrule
        4 & 赵 & 赵和钱 & 赵, 钱和孙 & 赵, 钱, 孙和李 \\
        \bottomrule[1.5pt]
    \end{tabular}
    \caption{mincitenames=1时，不同maxcitenames生成的标注。}
    \label{tab:mincitenames1}
\end{table*}
\begin{table*}[htb]
    \centering
    \begin{tabular}{ccccc}
        ...
        \bottomrule[1.5pt]
    \end{tabular}
    \caption{mincitenames=2时，不同maxcitenames生成的标注。}
    \label{tab:mincitenames2}
\end{table*}

其次可以设置正文中的标注显示多位作者时，最后两位作者间的连接符。本模板中文作者使用“和”连接，英文作者使用“and”连接：
\begin{Verbatim}
\DeclareDelimFormat[cite,textcite]{finalnamedelim}
{{\iffieldequalstr{userf}{chinese}{和}{\space and\space}}}
\end{Verbatim}

\subsection{参考文献表格式个性化配置}
首先可以自定义参考文献表中，每个条目显示的作者数量和大小写格式。
\begin{itemize}
    \item \texttt{\bfseries maxbibnames}：控制参考文献表中，每个条目最多显示的作者数量，超出部分会使用“等”省略。
    \item \texttt{\bfseries gbnamefmt}：控制参考文献表英文姓氏的大小写格式。
    \begin{itemize}
        \item lowercase：首字母大写（示例：Vector）。
        \item uppercase：全大写（示例：VECTOR）。
    \end{itemize}
\end{itemize}

其次可以设置参考文献表显示多位作者时，最后两位作者间的连接符。本模板中文作者使用“，和”连接，英文作者使用“, and”连接：
\begin{Verbatim}
\DeclareDelimFormat[bib,biblist]{finalnamedelim}
{{\iffieldequalstr{userf}{chinese}{，和}{, and\space}}}
\end{Verbatim}
...
例如，本文档中的“北京大学学位论文原创性声明和使用授权说明”一章就是用 \verb|\specialchap|这条命令开始的。%
